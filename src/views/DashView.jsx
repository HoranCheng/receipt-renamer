    ...
    <div style={{ padding: '0 16px 100px' }}>
      ...
      >
        ...
        <div style={{ fontSize: 11, color: T.tx3, textAlign: 'right' }}>
          <StatusDot level="ok" />
          <span style={{ marginLeft: 5 }}>{'\u5DF2\u8FDE\u63A5'}</span>
        </div>
      </div>
        ...
        >
          ...
          {receipts.length > 0 && (
            <div style={{ fontSize: 10, color: T.tx3, marginTop: 2 }}>
              {'\u5747'} ${(total / receipts.length).toFixed(0)}/{'\u7B14'}
            </div>
          )}
        </div>
      </div>
          ...
          {topCats.map(([cat, amt]) => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  <span style={{ fontSize: 12, color: T.tx, fontWeight: 500 }}>
                    {cat}
                  </span>
                  <span style={{ fontSize: 12, color: T.tx2, fontFamily: FM }}>
                    ${amt.toFixed(0)}
                  </span>
                </div>
                <div
      ...
      >
        <span style={{ fontSize: 11, fontWeight: 700, color: T.tx2 }}>
          {'\u6700\u8FD1\u5904\u7406'}
        </span>
        {receipts.length > 4 && (
          <button
      ...
      {recent.length === 0 ? (
        <div
          style={{ textAlign: 'center', padding: '36px 16px', color: T.tx3 }}
        >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          {recent.map((r) => (
            <ReceiptRow key={r.id} r={r} compact />
          ))}
        </div>
      )}
    </div>
